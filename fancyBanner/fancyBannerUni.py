...
class FancyBannerUni(object):
    ...
    def ribbon(self, msg='', width=0):
        return '{h}{msg:{h}^{width}}{h}'.format(width=width,
                                         msg=msg,
                                         h=self.box['h'],
                                         ) + '\n'
...
class SmallRibbon(FancyBannerUni):
    ...
    def __str__(self):
        ...
        to_str = '\n'
        to_str += '{msg:^{width}}'.format(msg=self.ribbon(self.messages[0], width=self.innerSize), width=self.width)
